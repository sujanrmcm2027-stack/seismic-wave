...
function DamageAssessmentPage() {
  ...
  const { events } = useLiveNepalEarthquakes();
  ...
  useEffect(() => {
    // Fetch real live incidents generated by the GitHub Action
    const fetchLiveIncidents = async () => {
      try {
        const response = await fetch("/live_incidents.json?t=" + Date.now(), { cache: "no-store" });
        if (response.ok) {
          ...
          if (Array.isArray(liveData) && liveData.length > 0) {
            setIncidents((prev) => {
              ...
              const newIncidents = liveData.filter(d => !existingIds.has(d.id));
              return [...newIncidents, ...prev].slice(0, MAX_FEED_LENGTH * 2);
            });
          }
        }
      } catch (e) {
    ...
    const fetchTimer = window.setInterval(fetchLiveIncidents, 5 * 60 * 1000);
    ...
    const timer = window.setInterval(() => {
      ...
      const incoming: DamageIncident = {
        ...generateLiveIncident(),
        id: `live-sim-${Date.now()}-${counter}`,
        time: new Date().toISOString(),
      };
      setIncidents((prev) => [incoming, ...prev].slice(0, MAX_FEED_LENGTH * 2));
    }, NEW_ALERT_INTERVAL_MS * 3); // Slowed down since we have real data
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 md:px-8">
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[65fr_35fr]">
          <section aria-label="Interactive damage map">
            <DamageAssessmentMap incidents={filteredIncidents} events={events} />
          </section>
